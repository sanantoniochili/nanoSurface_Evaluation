...
if( jgraph.vertexSet().size() > 1000 )
    ...
    public Vector<Pair<String,Double>> vectorExtract() {
        ...
        vector.addElement(new Pair<>("IndependentSet", (double) IndependentSet()));
    ...
    int cliques() {
        ...
        BronKerboschCliqueFinder<Object, DefaultEdge> cf = new BronKerboschCliqueFinder<>(graph);
    ...
    int IndependentSet() {
        ChordalGraphIndependentSetFinder finder = new ChordalGraphIndependentSetFinder(graph);

        return finder.getIndependentSet().size();
    }
